...
func TestRunCmd(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cmdOpts: []func(*RunCmd){
				func(c *RunCmd) { c.NoFlatten = true },
			},
			wantEnv: map[string]string{
	...
	for _, tc := range tests {
		t.Run(tc.id, func(t *testing.T) {
			mb, appCtx := newRunTestContext(t)
			tc.setup(mb)

			mr := &MockRunner{exitCode: 0}
			cmd := setupRunCmd(tc.from, tc.args, tc.cmdOpts...)
...
func TestRunCmd_EdgeCases(t *testing.T) {
	t.Run("RE-06-empty-prefix", func(t *testing.T) {
		...
		mr := &MockRunner{exitCode: 0}
		cmd := setupRunCmd([]string{"ps:/empty/prefix/"}, []string{"env"})
		cmd.runner = mr
	...
	t.Run("RE-07-duplicate-key-last-wins", func(t *testing.T) {
		...
		mr := &MockRunner{exitCode: 0}
		cmd := setupRunCmd([]string{"ps:/a/", "ps:/b/"}, []string{"env"})
		cmd.runner = mr
	...
	t.Run("RE-08-special-chars", func(t *testing.T) {
		...
		mr := &MockRunner{exitCode: 0}
		cmd := setupRunCmd([]string{"ps:/app/prod/"}, []string{"printenv", "MSG"})
		cmd.runner = mr
