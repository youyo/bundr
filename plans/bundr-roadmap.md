...
| 作成日 | 2026-02-27 |
| 最終更新 | 2026-02-27 12:30 |
| ステータス | 進行中 |

## Current Focus
- **マイルストーン**: M1 - プロジェクト骨格 + put/get コア実装
- **直接の完了**: ロードマップ・M1詳細計画作成
- **次のアクション**: M1 実装開始 (`/team` で実装チームを起動)

## Progress

### M1: プロジェクト骨格 + put/get コア実装
- [ ] go.mod 初期化 (module: github.com/youyo/bundr)
- [ ] ディレクトリ構造作成 (cmd/, internal/backend/, internal/config/, internal/tags/)
- [ ] Backend インターフェース定義 + mock 実装
- [ ] SSM Parameter Store バックエンド実装 (ps, psa)
- [ ] Secrets Manager バックエンド実装 (sm)
- [ ] `bundr put` コマンド実装 (raw/json mode, tag schema)
- [ ] `bundr get` コマンド実装 (cli-store-mode tag 参照)
- [ ] 単体テスト (TDD: mock backend で AWS 隔離)
